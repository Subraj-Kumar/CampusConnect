...
import EventCalendar from "../components/EventCalendar"; // The new Calendar component
...
const Events = () => {
  ...
  // --- 2. Data Fetching (Robust & Memoized) ---
  const fetchData = useCallback(async () => {
    let sliderData = [];

    // Fetch Slider Events (Fail gracefully if error)
    try {
    ...
    try {
      ...
      // Check Registration Status (For Students Only)
  ...
  return (
    <div className="events-page-container">
      {/* 🎨 Built-in Styles for Layout and UI */}
      <style>{`
        ...
        @media (min-width: 1024px) {
          .homepage-main-grid {
            grid-template-columns: 350px 1fr; /* 350px Calendar, remaining space for Grid */
            align-items: start;
          }
        }
        ...
        .card-title { font-size: 1.25rem; font-weight: 800; color: #1f2937; margin: 0 0 0.5rem 0; line-height: 1.2; }
        .card-desc { color: #6b7280; font-size: 0.875rem; margin-bottom: 1.5rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
      ...
      {/* 🟢 TWO-COLUMN LAYOUT (Calendar + Grid) */}
      <div className="homepage-main-grid">
        
        {/* LEFT COLUMN: Calendar Widget */}
        <aside>
          <EventCalendar events={[...sliderEvents, ...upcomingEvents]} />
        </aside>
